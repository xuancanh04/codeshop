    ...
    <header className="sticky top-0 z-50 overflow-visible border-b border-slate-200/80 bg-white/90 backdrop-blur-md dark:border-white/[0.06] dark:bg-slate-950/80 dark:backdrop-blur-xl">
      <div className="mx-auto flex min-w-0 max-w-6xl items-center justify-between gap-2 overflow-visible px-4 py-3 sm:gap-3 sm:px-6 lg:gap-4 lg:px-8">
        <BrandLogo
          ...
          className="relative z-[1] shrink-0 overflow-visible rounded-xl outline-none transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-blue-500/40 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-blue-400/40 dark:focus-visible:ring-offset-slate-950"
        />

        <nav className="hidden min-w-0 flex-1 flex-nowrap items-center justify-center gap-0.5 overflow-x-auto [scrollbar-width:none] md:flex lg:gap-1 [&::-webkit-scrollbar]:h-0 [&::-webkit-scrollbar]:w-0">
      ...
      {open && (
        <div className="border-t border-slate-100 bg-white px-4 py-4 dark:border-slate-800 dark:bg-slate-950 md:hidden">
          ...
          <nav className="flex flex-col gap-1">
            ...
            {isAdmin && (
              <NavLink to="/admin/products" className={navLinkClass} onClick={() => setOpen(false)}>
                {t('nav.admin')}
              </NavLink>
            )}
            <button
